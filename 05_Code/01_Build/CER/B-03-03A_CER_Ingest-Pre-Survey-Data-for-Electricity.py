...
> Purpose of Script
  : Ingest Residential Pre Survey Dataset for Electricity
...
FILE_TO_LOAD_CER_SURVEY_ELECTRICITY_PRE = \
    'Smart meters Residential pre-trial survey data.csv'
PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_PRE = os.path.join(
...
FILE_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_PRE = \
    'RESIDENTIAL-PRE-TRIAL-SURVEY_Modified.xlsx'
PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_PRE = os.path.join(
    HD.PATH_DATA_RAW_USE_CER_ELECTRICITY,
...
FILE_TO_SAVE_CER_SURVEY_ELECTRICITY_PRE = \
    'CER_Survey_Electricity_Pre.parquet'
PATH_TO_SAVE_CER_SURVEY_ELECTRICITY_PRE = os.path.join(
...
df_survey_electricity_pre_data = (
    sqlc.read
        .format('csv')
        .option('header', True)
        .load(PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_PRE)
)


# # 2. Rename columns
# # 2.1. Make a list of new names
names_questions_old = df_survey_electricity_pre_data.columns[1:]
...
# # 2.3.1. For `ID` column
df_survey_electricity_pre_data = \
    df_survey_electricity_pre_data.withColumnRenamed('ID', 'id')
# # 2.3.2. For remaining columns
df_survey_electricity_pre_data = functools.reduce(
    lambda df_survey_electricity_pre_data, idx:
    df_survey_electricity_pre_data.withColumnRenamed(
...
pdf_survey_electricity_pre_data = df_survey_electricity_pre_data.toPandas()

# # 3.2. Melt the Pandas DF
id_vars = ['id']
value_vars = [
    col for col in df_survey_electricity_pre_data.columns if col not in id_vars
]
pdf_survey_electricity_pre_data_melted = \
    pdf_survey_electricity_pre_data.melt(
        id_vars=id_vars, value_vars=value_vars,
        var_name='question_id', value_name='answer_code'
    )

# # 3.3. Transform the melted Pandas DF to a Spark DF
df_survey_electricity_pre_data_melted = \
    spark.createDataFrame(pdf_survey_electricity_pre_data_melted)
...
df_survey_electricity_pre_data_melted = \
    df_survey_electricity_pre_data_melted.withColumn('timing', lit('pre'))
...
df_survey_electricity_pre_data_melted = (
    df_survey_electricity_pre_data_melted
        ...
        [
            when(col(c) == "", None)
                .otherwise(col(c))
                .alias(c) for c in df_survey_electricity_pre_data_melted.columns
        ]
    )
)
...
df_survey_electricity_pre_data_melted = (
    functools.reduce(
        lambda df_survey_electricity_pre_data_melted, idx:
        df_survey_electricity_pre_data_melted.withColumn(
...
pdf_survey_electricity_pre_doc = pd.read_excel(
    PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_PRE,
...
for c in cols_convert:
    pdf_survey_electricity_pre_doc[c] = \
        pdf_survey_electricity_pre_doc[c].replace(' ', '-999')
    pdf_survey_electricity_pre_doc[c] = \
        pdf_survey_electricity_pre_doc[c].replace(np.nan, '-999')

# # 2.2. Remove whitespaces
# ## Note:
# ## This task is necessary to avoid any error in the middle of joining work.
pdf_survey_electricity_pre_doc['q_no'] = \
    pdf_survey_electricity_pre_doc['q_no'].apply(lambda row: row.strip())
pdf_survey_electricity_pre_doc['answer_code'] = (
    pdf_survey_electricity_pre_doc['answer_code']
...
pdf_survey_electricity_pre_doc['answer_desc'] = (
    pdf_survey_electricity_pre_doc['answer_desc']
        .apply(lambda row: ' ' + str(row))
)

# # 3. Create a Spark DF from the Pandas DF above
df_survey_electricity_pre_doc = \
    spark.createDataFrame(pdf_survey_electricity_pre_doc)
...
df_survey_electricity_pre_doc = (
    functools.reduce(
        lambda df_survey_electricity_pre_doc, idx:
            df_survey_electricity_pre_doc.withColumn(
                cols_convert[idx], col(cols_convert[idx]).cast(IntegerType())
            ),
            range(len(cols_convert)),
            df_survey_electricity_pre_doc
...
# # 2. Join DFs
df_survey_electricity_pre = (
    df_survey_electricity_pre_data_melted
        .join(
            df_survey_electricity_pre_doc.select(
...
df_survey_electricity_pre = df_survey_electricity_pre.select(cols_toOrder)

# # 3.2. Re-order rows
cols_toOrder = ['id', 'question_id', 'answer_code']
df_survey_electricity_pre = \
    df_survey_electricity_pre.orderBy(cols_toOrder, ascending=True)
...
pdf_survey_electricity_pre = df_survey_electricity_pre.toPandas()
FNC.save_toPq(
    pdf_survey_electricity_pre,
    path=PATH_TO_SAVE_CER_SURVEY_ELECTRICITY_PRE,
    preserve_idx=False
)
