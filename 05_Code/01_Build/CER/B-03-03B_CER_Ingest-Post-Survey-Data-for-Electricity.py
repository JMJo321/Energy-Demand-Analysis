...
> Purpose of Script
  : Ingest Residential Post Survey Dataset for Electricity
...
# # 1.1. For survey data
DIR_TO_LOAD_CER_SURVEY_ELECTRICITY = \
    'CER_Electricity_Data/Survey data - CSV format'
FILE_TO_LOAD_CER_SURVEY_ELECTRICITY_POST = \
    'Smart meters Residential post-trial survey data.csv'
PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_POST = os.path.join(
...
DIR_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC = 'CER_Electricity_Documentation'
FILE_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_POST = \
    'RESIDENTIAL-POST-TRIAL-SURVEY_Modified.xlsx'
PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_POST = os.path.join(
    HD.PATH_DATA_RAW_USE_CER_ELECTRICITY,
    DIR_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC,
    FILE_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_POST
)


# # 2. Path(s) at which data file(s) will be saved
FILE_TO_SAVE_CER_SURVEY_ELECTRICITY_POST = \
    'CER_Survey_Electricity_Post.parquet'
PATH_TO_SAVE_CER_SURVEY_ELECTRICITY_POST = os.path.join(
...
df_survey_electricity_post_data = (
    sqlc.read
        .format('csv')
        .option('header', True)
        .load(PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_POST)
)


# # 2. Rename columns
# # 2.1. Make a list of new names
names_questions_old = df_survey_electricity_post_data.columns[1:]
# ## Note: To exclude the name of the first column.
names_questions_id = []
...
for name in names_questions_old:
    ...
    q_desc = re.sub('^Q[a-z]+\s[0-9]+:\s', '', name)
...
# # 2.3.1. For `ID` column
df_survey_electricity_post_data = \
    df_survey_electricity_post_data.withColumnRenamed('ID', 'id')
# # 2.3.2. For remaining columns
df_survey_electricity_post_data = functools.reduce(
    lambda df_survey_electricity_post_data, idx:
    df_survey_electricity_post_data.withColumnRenamed(
...
pdf_survey_electricity_post_data = df_survey_electricity_post_data.toPandas()

# # 3.2. Melt the Pandas DF
id_vars = ['id']
value_vars = [
    col for col in df_survey_electricity_post_data.columns if col not in id_vars
]
pdf_survey_electricity_post_data_melted = \
    pdf_survey_electricity_post_data.melt(
        id_vars=id_vars, value_vars=value_vars,
        var_name='question_id', value_name='answer_code'
    )

# # 3.3. Transform the melted Pandas DF to a Spark DF
df_survey_electricity_post_data_melted = \
    spark.createDataFrame(pdf_survey_electricity_post_data_melted)
...
df_survey_electricity_post_data_melted = (
    df_survey_electricity_post_data_melted
        .withColumn('utility', lit('electricity'))
)

# # 1.2. Add a column showing the data imported is for post survey
df_survey_electricity_post_data_melted = \
    df_survey_electricity_post_data_melted.withColumn('timing', lit('post'))
...
df_survey_electricity_post_data_melted = (
    df_survey_electricity_post_data_melted
        ...
        [
            when(col(c) == "", None)
                .otherwise(col(c))
                .alias(c) for c in df_survey_electricity_post_data_melted.columns
        ]
    )
)
...
df_survey_electricity_post_data_melted = (
    functools.reduce(
        lambda df_survey_electricity_post_data_melted, idx:
        df_survey_electricity_post_data_melted.withColumn(
...
pdf_survey_electricity_post_doc = pd.read_excel(
    PATH_TO_LOAD_CER_SURVEY_ELECTRICITY_DOC_POST,
...
for c in cols_convert:
    pdf_survey_electricity_post_doc[c] = \
        pdf_survey_electricity_post_doc[c].replace(' ', '-999')
    pdf_survey_electricity_post_doc[c] = \
        pdf_survey_electricity_post_doc[c].replace(np.nan, '-999')

# # 2.2. Remove whitespaces
# ## Note:
# ## This task is necessary to avoid any error in the middle of joining work.
pdf_survey_electricity_post_doc['q_no'] = \
    pdf_survey_electricity_post_doc['q_no'].apply(lambda row: row.strip())
pdf_survey_electricity_post_doc['answer_code'] = (
    pdf_survey_electricity_post_doc['answer_code']
...
pdf_survey_electricity_post_doc['answer_desc'] = (
    pdf_survey_electricity_post_doc['answer_desc']
        .apply(lambda row: ' ' + str(row))
)

# # 3. Create a Spark DF from the Pandas DF above
df_survey_electricity_post_doc = \
    spark.createDataFrame(pdf_survey_electricity_post_doc)
...
df_survey_electricity_post_doc = (
    functools.reduce(
        lambda df_survey_electricity_post_doc, idx:
            df_survey_electricity_post_doc.withColumn(
                cols_convert[idx], col(cols_convert[idx]).cast(IntegerType())
            ),
            range(len(cols_convert)),
            df_survey_electricity_post_doc
...
# # 2. Join DFs
df_survey_electricity_post = (
    df_survey_electricity_post_data_melted
        .join(
            df_survey_electricity_post_doc.select(
...
df_survey_electricity_post = df_survey_electricity_post.select(cols_toOrder)

# # 3.2. Re-order rows
cols_toOrder = ['id', 'question_id', 'answer_code']
df_survey_electricity_post = \
    df_survey_electricity_post.orderBy(cols_toOrder, ascending=True)
...
pdf_survey_electricity_post = df_survey_electricity_post.toPandas()
FNC.save_toPq(
    pdf_survey_electricity_post,
    path=PATH_TO_SAVE_CER_SURVEY_ELECTRICITY_POST,
    preserve_idx=False
)
