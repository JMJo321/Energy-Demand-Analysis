...
dt_for.reg[
  ,
  range_hdd := cut(hdd, breaks = seq(0, 35, by = 1), include.lowest = TRUE)
]
...
dt_avg.kwh_daily <-
  dt_for.reg[   # To obtain each household's daily consumption
    ...
    by = .(date, is_treated_r, is_treatment.period, range_hdd)
  ]
# ## Note:
...
# # 1.3.1. Add columns
# # 1.3.1.1. A column that shows treatment groups
dt_avg.kwh_daily[is_treated_r == TRUE, group := "Treatment Group"]
dt_avg.kwh_daily[is_treated_r == FALSE, group := "Control Group"]
# # 1.3.1.2. A column that show treatment periods
dt_avg.kwh_daily[is_treatment.period == TRUE, period := "Treatment"]
dt_avg.kwh_daily[is_treatment.period == FALSE, period := "Baseline"]
...
dt_simulation_fes_daily_excl.control_linear <-
  data.table(hdd = seq(0, 35, by = 0.5)) %>%
    .[
      ,
      `:=` (
        model = "Linear",
        response = (
          dt_fes_daily_excl.control_linear[
            ...
            dt_fes_daily_excl.control_linear[
              str_detect(desc, "^hdd:treatment.and.post")
            ]$estimate * hdd
        )
      )
    ]
...
# # 1. Create a Plot that shows Daily Average Consumption
plot_avg.kwh_daily <-
...
# # 2. Create a Plot that shows Simulation Results
plot_simulation_fes <-
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Simulation_Response-to-Temperature_Daily_Electricity.png",
    sep = "/"
  ),
  plot_simulation_fes,
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Daily-Average-Consumption_By-HDD_Electricity.png",
    sep = "/"
  ),
  plot_avg.kwh_daily,
  width = 40, height = 30, units = "cm"
