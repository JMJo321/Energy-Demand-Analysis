...
FILE_TO.SAVE_CER_ESTIMATES <- "CER_Estimates_Response-to-Temperature.RData"
...
reg.results <-
  files[str_detect(files, "^CER_Regression-Results_Response-to-Temperature")]
...
for (result in reg.results) {
  ...
  tmp_obj.name <-
    str_replace(
      result, "^CER_Regression-Results_Response-to-Temperature_", ""
    ) %>%
      str_replace(., "\\.RData", "") %>%
      str_replace_all(., "-", ".") %>%
      tolower(.) %>%
      paste0("estimates_temp.response_30min_", .)
