...
# # household response to temperature
...
results_temp.response_30min_i_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[1]]
)
save(
  results_temp.response_30min_i_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_i.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_i_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 2. For the second model
results_temp.response_30min_i.d_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[2]]
)
save(
  results_temp.response_30min_i.d_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_i-d.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_i.d_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 3. For the third model
results_temp.response_30min_iw.d_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[3]]
)
save(
  results_temp.response_30min_iw.d_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_iw-d.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_iw.d_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 4. For the fourth model
results_temp.response_30min_iw.dw_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[4]]
)
save(
  results_temp.response_30min_iw.dw_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_iw-dw.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_iw.dw_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 5. For the fifth model
results_temp.response_30min_iw.dw.m_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[5]]
)
save(
  results_temp.response_30min_iw.dw.m_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_iw-dw-m.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_iw.dw.m_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 6. For the sixth model
results_temp.response_30min_iw.dw.mp_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[6]]
)
save(
  results_temp.response_30min_iw.dw.mp_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_iw-dw-mp.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_iw.dw.mp_clustered.ses)
gc(reset = TRUE, full = TRUE)


# # 7. For the seventh model
results_temp.response_30min_iw.dw.mpw_clustered.ses <- lapply(
  list_cases,
  get_reg.result,
  formula = modified.models_temp.response_30min_with.clustered.ses[[7]]
)
save(
  results_temp.response_30min_iw.dw.mpw_clustered.ses,
  file = paste(
    DIR_TO.SAVE_CER,
    paste0(
      "CER_Regression-Results_Response-to-Temperature_",
      "By-Season-and-Rate-Period_iw.dw.mpw.RData"
    ),
    sep = "/"
  )
)
rm(results_temp.response_30min_iw.dw.mpw_clustered.ses)
gc(reset = TRUE, full = TRUE)
