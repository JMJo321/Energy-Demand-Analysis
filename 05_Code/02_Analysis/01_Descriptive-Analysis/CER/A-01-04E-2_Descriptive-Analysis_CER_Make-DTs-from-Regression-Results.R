...
FILE_TO.SAVE_CER_ESTIMATES <- "CER_Estimates_Average-Treatment-Effect.RData"
...
for (result in reg.results) {
  ...
  tmp_obj.name <-
    str_replace(
      result, "^CER_Regression-Results_Average-Treatment-Effect_", ""
    ) %>%
      str_replace(., "\\.RData", "") %>%
      str_replace_all(., "-", ".") %>%
      tolower(.) %>%
      paste0("estimates_avg.effect_30min_", .)
