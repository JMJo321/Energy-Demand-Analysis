...
dt_estimates_by.season <- rbind(
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_i,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_i.d,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_id.d,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-d + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_ip.d,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-p + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_idp.d,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-d-BY-p + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_idp.dp,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-d-BY-p + d-BY-p"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_idp.dp.m,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-d-BY-p + d-BY-p + m"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response.by.period_rate.period_by.season_idp.dp.mp,
    idcol = "season"
  ) %>%
    .[, model := "FEs: i-BY-d-BY-p + d-BY-p + m-BY-p"] %>%
    .[!is.na(estimate)]
) %>%
  .[
    ,
    model := factor(
      model,
      levels = c(
        "FEs: i", "FEs: i + d", "FEs: i-BY-d + d", "FEs: i-BY-p + d",
        "FEs: i-BY-d-BY-p + d", "FEs: i-BY-d-BY-p + d-BY-p",
        "FEs: i-BY-d-BY-p + d-BY-p + m", "FEs: i-BY-d-BY-p + d-BY-p + m-BY-p"
      )
    )
  ]
...
plot_by.season_reduction <-
  ggplot(
    ...
    labs(
      x = "Rate Period",
      y = "Estimated Response to Temperature: Reduction  (kWh per Hour)",
      shape = "Significant?",
      caption = paste0(
        "Note: i - Household, d - Day of Week, ",
...
plot_by.season_slope <-
  ggplot(
    ...
    labs(
      x = "Rate Period",
      y = "Estimated Response to Temperature: Change in Slope  (kWh per Hour)",
      shape = "Significant?",
      caption = paste0(
        "Note: i - Household, d - Day of Week, ",
        "m - Month of Year, and p - Rate Period"
...
plot_simulation_by.season <-
  ggplot(data = dt_simulation_by.season) +
    ...
    labs(
      x = "HDDs",
      y = "Predicted Consumption  (kWh per Hour)",
      color = "Rate Periods",
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Rate-Period-Level-Response-to-Temperature_Average-kWh_Reduction-only.png",
    sep = "/"
  ),
  plot_by.season_reduction,
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Rate-Period-Level-Response-to-Temperature_Average-kWh_Slope-only.png",
    sep = "/"
  ),
  plot_by.season_slope,
  width = 45, height = 50, units = "cm"
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Rate-Period-Level-Response-to-Temperature_Average-kWh_Simulation.png",
    sep = "/"
  ),
  plot_simulation_by.season,
