...
FILE_TO.LOAD_CER_ESTIMATES <- "CER_Estimates_Response-to-Temperature.RData"
...
dt_estimates_by.season.and.rate.period <- rbind(
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_i,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_i.d,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_iw.d,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i-BY-w + d"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_iw.dw,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_iw.dw.m,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w + m"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_iw.dw.mp,
    idcol = "season.and.rate.period"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w + m-BY-p"] %>%
    .[!is.na(estimate)],
  rbindlist(
    estimates_temp.response_30min_by.season.and.rate.period_iw.dw.mpw,
    idcol = "season.and.rate.period"
  ) %>%
...
plot_by.season.and.rate.period_slope <-
  ggplot(
    ...
    facet_grid(model ~ season, scales = "free_x") +
    scale_x_continuous(breaks = seq(0, 24, by = 1)) +
    scale_shape_manual(values = c(1, 21)) +
    labs(
      x = "Hour of Day",
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Response-to-Temperature_Reduction-only.png",
    sep = "/"
  ),
  plot_by.season.and.rate.period_reduction,
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Response-to-Temperature_Slope-only.png",
    sep = "/"
  ),
  plot_by.season.and.rate.period_slope,
  width = 60, height = 45, units = "cm"
...
plot.save(
  paste(
    DIR_TO.SAVE_PLOT,
    "CER_Response-to-Temperature_Simulation.png",
    sep = "/"
  ),
  plot_simulation_by.season.and.rate.period,
