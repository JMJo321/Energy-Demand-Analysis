...
for (result in reg.results) {
  ...
  tmp_obj.name <-
    str_replace(
      ...
      tolower(.) %>%
      paste0(
        "estimates_treatment.effect_by.season.and.tariff_", tmp_ref.temp, .
      )
...
# # 1.1. For Estimates regarding the reference temperature of 60 degrees
# #      Fahrenheit
dt_estimates_treatment.effect_by.season.and.tariff_based.on.60f <- rbind(
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.dp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.dw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.mp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + m-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.mw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + m-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.dp.mp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-p + m-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.60f_iw.dw.mw,
    idcol = "tmp_desc"
  ) %>%
...
# # 1.2. For Estimates regarding the reference temperature of 65 degrees
# #      Fahrenheit
dt_estimates_treatment.effect_by.season.and.tariff_based.on.65f <- rbind(
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.dp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.dw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.mp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + m-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.mw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + m-BY-w"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.dp.mp,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-p + m-BY-p"],
  rbindlist(
    estimates_treatment.effect_by.season.and.tariff_based.on.65f_iw.dw.mw,
    idcol = "tmp_desc"
  ) %>%
    .[, model := "FEs: i-BY-w + d-BY-w + m-BY-w"]
) %>%
...
# # 1.3. Bind the two DTs
dt_estimates_treatment.effect_by.season.and.tariff <- rbind(
  dt_estimates_treatment.effect_by.season.and.tariff_based.on.60f,
  dt_estimates_treatment.effect_by.season.and.tariff_based.on.65f
)
