...
models <- list(
  model_ate_wo.hdd.term = model_ate_wo.hdd.term,
  model_ate_w.hdd.term = model_ate_w.hdd.term
)
...
for (idx in 1:4) {
  ...
  tmp_file.name <- paste(
    PATH_DATA_ANALYSIS, "04_CER/From-Scratches/2021-11-15",
    paste0(
...
for (idx in 1) { # Rate code is not required.
  ...
  assign(
    tmp_obj.name,
    get_reg.result(list_cases[[idx]][1:2], model_rate.change_wo.hdd.term)
  )
  save(list = tmp_obj.name, file = tmp_file.name)
  rm(list = tmp_obj.name)
  gc(reset = TRUE)
...
for (idx in 1) {
  ...
  assign(
    tmp_obj.name,
    get_reg.result(list_cases[[idx]][1:2], model_rate.change_w.hdd.term)
  )
  save(list = tmp_obj.name, file = tmp_file.name)
  rm(list = tmp_obj.name)
  gc(reset = TRUE)
...
# TODO: 3.1. Load files
path_to.load <- paste(
  PATH_DATA_ANALYSIS, "04_CER/From-Scratches/2021-11-15", sep = "/"
)
...
dt_simulation_static_rate.change[
  ,
  `:=` (
    kwh_temp = estimate_temp * hdd * rate.change,
    kwh_non.temp = estimate_non.temp * rate.change
  )
]
